# Modified for GeoEdit dual-branch denoising.
# Based on ModelScope DiffSynth-Studio, licensed under Apache-2.0.
import torch, types
...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        ttm_disable_vhi: Optional[bool] = False,
        ...
        inputs_shared = {
            ...
            "enable_ttm": enable_ttm, "motion_signal_video": motion_signal_video, "motion_signal_mask": motion_signal_mask, "ttm_mask_old": ttm_mask_old,
        ...
        ttm_enabled = inputs_shared.get("ttm_enabled", False)
        ttm_start_index = inputs_shared.get("ttm_start_index", 0) if ttm_enabled else 0
        timesteps = self.scheduler.timesteps

        def _align_temporal_cond(cond_tensor, target_tensor, pad_value=0.0):
            if cond_tensor is None or target_tensor is None:
                return cond_tensor
            if cond_tensor.shape[2] == target_tensor.shape[2]:
                return cond_tensor
            if cond_tensor.shape[2] < target_tensor.shape[2]:
                prefix = target_tensor.shape[2] - cond_tensor.shape[2]
                pad = torch.full(
                    (cond_tensor.shape[0], cond_tensor.shape[1], prefix, cond_tensor.shape[3], cond_tensor.shape[4]),
                    pad_value,
                    device=cond_tensor.device,
                    dtype=cond_tensor.dtype,
                )
                return torch.cat([pad, cond_tensor], dim=2)
            return cond_tensor[:, :, : target_tensor.shape[2]]

        for progress_id, timestep in enumerate(progress_bar_cmd(timesteps[ttm_start_index:])):
            global_step = progress_id + ttm_start_index
            # Expose the current denoising step for analysis/debug hooks.
            self.debug_global_step = global_step
            self.debug_timestep_value = timesteps[global_step]
            self.debug_total_steps = len(timesteps)
            ...
            if getattr(self, "debug_stop_after_capture", False) and getattr(self, "debug_capture_complete", False):
                break
            if cfg_scale != 1.0:
                if cfg_merge:
            ...
            # Scheduler
            inputs_shared["latents"] = self.scheduler.step(noise_pred, timesteps[global_step], inputs_shared["latents"])

            if ttm_enabled and inputs_shared.get("ttm_ref_latents") is not None:
                ...
                replace_new_region = replace_start <= global_step < ttm_tstrong
                if replace_new_region:
                    ttm_disable_vhi = inputs_shared.get("ttm_disable_vhi", False)
                    if ttm_disable_vhi:
                        noisy_ref = inputs_shared["ttm_ref_latents"].to(
                            dtype=inputs_shared["latents"].dtype, device=inputs_shared["latents"].device
                        )
                    else:
                        next_idx = global_step + 1
                        if next_idx < len(timesteps):
                            next_t = timesteps[next_idx]
                            noisy_ref = self.scheduler.add_noise(
                                inputs_shared["ttm_ref_latents"],
                                inputs_shared["ttm_fixed_noise"],
                                next_t,
                            ).to(dtype=inputs_shared["latents"].dtype, device=inputs_shared["latents"].device)
                        else:
                            noisy_ref = inputs_shared["ttm_ref_latents"].to(
                                dtype=inputs_shared["latents"].dtype, device=inputs_shared["latents"].device
                            )
                    if replace_new_region:
                        replace_mode = inputs_shared.get("ttm_replace_mode", "mask_new")
                        if replace_mode == "non_hole":
                            hole_mask = inputs_shared.get("ttm_mask_old")
                            replace_mask = None if hole_mask is None else (1.0 - hole_mask)
                            if replace_mask is None:
                                replace_mask = inputs_shared.get("ttm_motion_mask")
                        else:
                            replace_mask = inputs_shared.get("ttm_motion_mask")
                        if replace_mask is not None:
                            replace_mask = _align_temporal_cond(replace_mask, inputs_shared["latents"], pad_value=0.0)
                            inputs_shared["latents"] = (
                                inputs_shared["latents"] * (1.0 - replace_mask) + noisy_ref * replace_mask
                            )
                        else:
                            background_mask = _align_temporal_cond(
                                inputs_shared["ttm_background_mask"], inputs_shared["latents"], pad_value=1.0
                            )
                            motion_mask = _align_temporal_cond(
                                inputs_shared["ttm_motion_mask"], inputs_shared["latents"], pad_value=0.0
                            )
                            inputs_shared["latents"] = (
                                inputs_shared["latents"] * background_mask
                                + noisy_ref * motion_mask
                            )
            if "first_frame_latents" in inputs_shared:
                inputs_shared["latents"][:, :, 0:1] = inputs_shared["first_frame_latents"]

        # Analysis-only fast exit: when a debug hook has already captured what it
        # needs, skip the remaining decode path entirely.
        if getattr(self, "debug_stop_after_capture", False) and getattr(self, "debug_capture_complete", False):
            return getattr(self, "debug_capture_payload", None)
...
class WanVideoUnit_TTM(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=(
                "enable_ttm",
    ...
    @staticmethod
    def _coerce_video(video, height, width):
        if isinstance(video, str):
            from diffsynth.utils.data import VideoData
            return VideoData(video, height=height, width=width)
        ...
        if isinstance(video, list) and len(video) > 0 and isinstance(video[0], Image.Image):
            return [frame.resize((width, height)) for frame in video]
        return video

    @staticmethod
    def _limit_length(video, length):
        if hasattr(video, "set_length"):
            video.set_length(length)
            return video
    ...
    @staticmethod
    def _mask_to_latent(mask_t1_hw, temporal_downsample, spatial_downsample):
        # mask_t1_hw: [T, 1, H, W] -> latent mask [1, 1, T_lat, H_lat, W_lat]
        mask0 = mask_t1_hw[0:1]
        mask1 = mask_t1_hw[1::temporal_downsample]
        sampled = torch.cat([mask0, mask1], dim=0)
        pooled = sampled.permute(1, 0, 2, 3).unsqueeze(0)
        h_lat = pooled.shape[-2] // spatial_downsample
        w_lat = pooled.shape[-1] // spatial_downsample
        pooled = F.interpolate(pooled, size=(pooled.shape[2], h_lat, w_lat), mode="nearest")
        return pooled
    ...
    def process(
        ...
        if not enable_ttm:
            return {"ttm_enabled": False}

        if motion_signal_video is None or motion_signal_mask is None:
            raise ValueError("`motion_signal_video` and `motion_signal_mask` are required when enable_ttm=True.")

        total_steps = len(pipe.scheduler.timesteps)
        tweak_index = 0 if tweak_index is None else int(tweak_index)
        use_warm_start = True if ttm_warm_start is None else bool(ttm_warm_start)
        if tweak_index < -1:
            raise ValueError(f"tweak_index ({tweak_index}) must be >= -1.")
        if tweak_index >= total_steps:
            raise ValueError(f"tweak_index ({tweak_index}) must be < {total_steps}.")
        replace_start_index = 0 if tweak_index < 0 else tweak_index
        ttm_start_index = replace_start_index if use_warm_start else 0
        ttm_tstrong_index = total_steps if tstrong_index is None else int(tstrong_index)
        if ttm_tstrong_index > total_steps:
            raise ValueError(f"tstrong_index ({ttm_tstrong_index}) must be <= {total_steps}.")
        if ttm_tstrong_index < max(0, tweak_index):
            raise ValueError(
        ...
        if len(motion_signal_mask) < num_frames:
            raise ValueError(f"motion_signal_mask has {len(motion_signal_mask)} frames, expected >= {num_frames}.")
        if len(motion_signal_video) > num_frames:
        ...
        if mask_old_provided:
            if isinstance(ttm_mask_old, Image.Image):
                ttm_mask_old = [ttm_mask_old]
            ttm_mask_old = self._coerce_video(ttm_mask_old, height, width)
            if hasattr(ttm_mask_old, "set_length"):
                if len(ttm_mask_old) < num_frames:
                    raise ValueError(f"ttm_mask_old has {len(ttm_mask_old)} frames, expected >= {num_frames}.")
                if len(ttm_mask_old) > num_frames:
                    ttm_mask_old.set_length(num_frames)
            else:
                if len(ttm_mask_old) == 1 and num_frames > 1:
                    ttm_mask_old = ttm_mask_old * num_frames
                elif len(ttm_mask_old) < num_frames:
                    raise ValueError(f"ttm_mask_old has {len(ttm_mask_old)} frames, expected >= {num_frames}.")
                elif len(ttm_mask_old) > num_frames:
                    ttm_mask_old = ttm_mask_old[:num_frames]
        ...
        ref_video = pipe.preprocess_video(motion_signal_video)
        ref_latents = pipe.vae.encode(
            ref_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride
        ).to(dtype=pipe.torch_dtype, device=pipe.device)

        mask_video = pipe.preprocess_video(motion_signal_mask, min_value=0, max_value=1)
        mask_tc_hw = mask_video[0].permute(1, 0, 2, 3).contiguous()  # [T, C, H, W]
        mask_t1_hw = (mask_tc_hw > 0.5).any(dim=1, keepdim=True).float()  # [T, 1, H, W]
        motion_mask = self._mask_to_latent(
            mask_t1_hw,
        ...
        if mask_old_provided:
            mask_old_video = pipe.preprocess_video(ttm_mask_old, min_value=0, max_value=1)
            mask_old_tc_hw = mask_old_video[0].permute(1, 0, 2, 3).contiguous()
            mask_old_t1_hw = mask_old_tc_hw.mean(dim=1, keepdim=True).clamp(0, 1)
            mask_old_latent = self._mask_to_latent(
                mask_old_t1_hw,
                temporal_downsample=pipe.time_division_factor,
                spatial_downsample=pipe.vae.upsampling_factor,
            ).to(dtype=pipe.torch_dtype, device=pipe.device)
            mask_old_latent = mask_old_latent.clamp(0, 1)

        if latents is not None:
            prefix = latents.shape[2] - ref_latents.shape[2]
            if prefix > 0:
                pad_latents = torch.zeros(
                    (ref_latents.shape[0], ref_latents.shape[1], prefix, ref_latents.shape[3], ref_latents.shape[4]),
                    device=ref_latents.device,
                    dtype=ref_latents.dtype,
                )
                ref_latents = torch.cat([pad_latents, ref_latents], dim=2)
                pad_mask = torch.zeros(
                    (motion_mask.shape[0], motion_mask.shape[1], prefix, motion_mask.shape[3], motion_mask.shape[4]),
                ...
                if mask_old_latent is not None:
                    pad_old = torch.zeros(
                        (mask_old_latent.shape[0], mask_old_latent.shape[1], prefix, mask_old_latent.shape[3], mask_old_latent.shape[4]),
                        device=mask_old_latent.device,
                        dtype=mask_old_latent.dtype,
                    )
                    mask_old_latent = torch.cat([pad_old, mask_old_latent], dim=2)
            elif prefix < 0:
        ...
        fixed_noise = pipe.generate_noise(
            ref_latents.shape,
            seed=seed,
        ...
        if use_warm_start and tweak_index >= 0:
            if ttm_initial_clean:
                warm_latents = ref_latents.to(dtype=pipe.torch_dtype, device=pipe.device)
            else:
                warm_t = pipe.scheduler.timesteps[ttm_start_index]
                warm_latents = pipe.scheduler.add_noise(ref_latents, fixed_noise, warm_t)
            if latents is not None:
                warm_latents = warm_latents.to(dtype=latents.dtype, device=latents.device)
            outputs["latents"] = warm_latents
...
class WanVideoUnit_VACE(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("vace_video", "vace_video_mask", "vace_reference_image", "vace_scale", "motion_signal_mask", "ttm_mask_old", "height", "width", "num_frames", "tiled", "tile_size", "tile_stride"),
    ...
    def process(
        self,
        pipe: WanVideoPipeline,
        vace_video, vace_video_mask, vace_reference_image, vace_scale, motion_signal_mask, ttm_mask_old,
        ...
        if vace_video is not None or vace_video_mask is not None or vace_reference_image is not None:
            ...
            # Depth latents are no longer zeroed after VAE encoding.
